...
function rigid_body_dynamics(position, quaternion, velocity, angular_vel, Δt)
    r = angular_vel #renamed for convenience
    mag = norm(r) #square root of rTr (useful in calculating the derivative)

    sᵣ = cos(mag * Δt / 2.0) #deriv w.r.t mag
    vᵣ = sin(mag * Δt / 2.0) * r / mag #dimensions 3x1

    sₙ = quaternion[1]
    vₙ = quaternion[2:4]

    s = sₙ * sᵣ - vₙ' * vᵣ # dimensions are 1x1
    v = sₙ * vᵣ + sᵣ * vₙ + vₙ × vᵣ # dimensions 3x1
...
function get_gps_transform()
    ...
    T = [R_gps_to_body t_gps_to_body] #3x4
end
...
function get_imu_transform()
    ...
    T = [R_imu_to_body t_imu_to_body] # 3x4
end
